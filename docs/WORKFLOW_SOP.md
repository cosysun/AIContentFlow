...
**目标**：对文章进行十维度全方位质检，确保达到A级发布标准。

**⚠️ 重要**：这是发布前的最后一道关卡，强烈建议执行！
...
```bash
# 读取文章
read_file /data/workspace/.draft/2026-02-24/article.md
...
**按照以下维度逐项检查**（详见 [QUALITY_CHECK_WORKFLOW.md](./QUALITY_CHECK_WORKFLOW.md)）：
...
```bash
# 可选：使用自动化脚本辅助
cd /data/workspace/AIContentFlow/tools
python3 quality_checker.py /data/workspace/.draft/2026-02-24/article.md
```
...
**保存位置**：
```bash
/data/workspace/.draft/2026-02-24/quality_check_report.md
```
...
##### Step 6: 使用notify推送质检结果

```json
{
  "title": "📋 质检完成 - {评级}",
  "message": "**总分**：{score}/100\n**评级**：{grade}\n\n{主要问题摘要}\n\n**详细报告**：.draft/{date}/quality_check_report.md\n\n{后续操作提示}"
}
```
...
|---------|---------|
| "确认发布" | 进入发布流程（阶段10） |
| "应用建议"/"应用方案" | 自动修复问题 → 重新质检 |
| "手动修改" | 等待用户修改 → 提示"修改完成后回复'重新质检'" |
| "重写[章节]" | 返回阶段3，针对性重写 |
| "查看报告" | 展示完整质检报告内容 |
| "忽略发布"/"强制发布" | 记录质检结果 → 进入发布流程 |
...
每次质检后更新统计：
```json
// /data/workspace/AIContentFlow/stats/quality_stats.json
{
  "total_checks": 10,
  "grade_distribution": {
    "A": 6,
...
**🔗 相关文档**：
- 完整质检标准和流程：[QUALITY_CHECK_WORKFLOW.md](./QUALITY_CHECK_WORKFLOW.md)
- 质检工具脚本：`/data/workspace/AIContentFlow/tools/quality_checker.py`
...
**目标**：将文章发布到多个平台并执行推广策略

**前置条件**：
- 质检已通过（A级）或用户确认忽略质检结果
- 配图已完成（如需要）
...
**保存位置**：`/data/workspace/.draft/YYYY-MM-DD/publish_plan.md`
...
**⚠️ 注意**：发布流程已整合到阶段10，本章节提供更详细的扩展说明。
